...
% \mcmTheme{DeepBlue}
 % 
% --- Metadata ---
\title[Mid-Century Modern]{Mid-Century Modern:\\A Retro-Inspired Template}
...
% --- Logo ---
% Your own logo goes here — give it a height, not a width: the footer strip is
% built around the logo's height. Delete this line and the scan lines run the
% full width of the strip.
\titlegraphic{\includegraphics[height=0.8cm]{logos/your-logo-kraft.pdf}}
% \titlegraphic{\includegraphics[height=0.8cm]{logos/your-logo-deepblue.pdf}}
...
\begin{frame}{Design Principles}
  \framesubtitle{A retro look, built on modern LaTeX}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Design Goals}
        \begin{itemize}
          \item Typographic dash bullets that follow the text size
...
\subsection{Blocks in Columns}

\begin{frame}{Blocks in Columns}
  \begin{block}{Overview}
    {\small A \textbf{standard block} spanning the full width can introduce
    the content below, before splitting into multiple columns.}
  \end{block}
  \begin{columns}[T]
    \begin{column}{0.31\textwidth}
      \begin{block}{Design}
        {\small
          \begin{itemize}
            \item \textbf{Typographic} dash bullets at every level
            \item Clean sans-serif typography
          \end{itemize}
        }
      \end{block}
    ...
    \begin{column}{0.31\textwidth}
      \begin{block}{Colour}
        {\small
          \begin{itemize}
            \item Two built-in themes: \textbf{Kraft} and \textbf{DeepBlue}
            \item Switched with a single command
          \end{itemize}
        }
      \end{block}
    \end{column}
    \begin{column}{0.31\textwidth}
      \begin{block}{Customisation}
        {\small
          \begin{itemize}
            \item Adjustable lengths and \textbf{font overrides}
            \item Compatible with \texttt{biblatex}
          \end{itemize}
        }
      \end{block}
    \end{column}
...
\begin{frame}{Alert and Example Blocks — Kraft theme}
  \framesubtitle{Amber for alerts, sage for examples}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Note}
        Standard blocks use the primary theme colour for the pip
...
\begin{frame}{Alert and Example Blocks — DeepBlue theme}
  \framesubtitle{Pink for alerts, mint for examples}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Note}
        Standard blocks use the primary theme colour for the pip
...
\begin{frame}{Text Formatting}
  \framesubtitle{Two families: one for the running text, one for the titles}
  Regular text uses the body font (TeX Gyre Heros, a Helvetica, by default).
  ...
  \vspace{0.5em}
  Frame titles use the title font (TeX Gyre Adventor, an Avant Garde
  Gothic, by default), letterspaced and rendered in uppercase.
...
\begin{frame}{Itemize and Enumerate}
  \framesubtitle{The mark follows the size and weight of its own text}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{itemize}
        \item First item
    ...
    \begin{column}{0.56\textwidth}
      \begin{block}{Lengths}
        \begin{itemize}
          ...
          \item \texttt{\textbackslash mcmFrameTitleHeight} — one-line title band
          \item \texttt{\textbackslash mcmFrameSubtitleGrow} — height a subtitle adds
          \item \texttt{\textbackslash mcmBlockTitleIndent} — block title indent
          \item \texttt{\textbackslash mcmBlockSep} — gap around blocks
          \item \texttt{\textbackslash mcmBandPad} — block title band padding
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.40\textwidth}
      \begin{block}{Lengths (cont.)}
        \begin{itemize}
          \item \texttt{\textbackslash mcmFrameTitleRise}
          \item \texttt{\textbackslash mcmTitleWordSpacing}
          \item \texttt{\textbackslash mcmFooterHeight}
          \item \texttt{\textbackslash mcmLogoVOffset}
        \end{itemize}
      \end{block}

      \begin{block}{Toggles}
        \begin{itemize}
          \item \texttt{\textbackslash mcmTitleUppercasefalse}
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
